...
function set_trajet_debug($transporteur,$veh,$parc,$dep,$vers,$debut,$fin,$penalite,$km,$chauff){
	...
	if (is_numeric($debut) && $debut > 0) {
		$debut_str = date('Y-m-d H:i:s', $debut);
	} else {
		$debut_str = date('Y-m-d H:i:s');
	}
	
	if (is_numeric($fin) && $fin > 0) {
		$fin_str = date('Y-m-d H:i:s', $fin);
	} else {
		$fin_str = date('Y-m-d H:i:s');
	}
	...
	$logfile = "logs/insert_debug.log";
	
	try {
		// Log input data
		$log = "═══════════════════════════════════\n";
		$log .= date("Y-m-d H:i:s") . " - DEBUG INSERT\n";
		$log .= "Vehicle: $veh | Parcour: $parc | Depart: $dep | Vers: $vers\n";
		$log .= "Debut: $debut_str | Fin: $fin_str | Penalite: $penalite | KM: $km\n";
		...
		$checkStmt->execute([$transporteur, $veh, $debut_str, $fin_str]);
		...
		$log = "SQL: $requete7\n";
		$log .= "Values: [$transporteur, '$veh', '$parc', '$dep', '$vers', '$debut_str', '$fin_str', $penalite, $km, '$chauff']\n";
		file_put_contents($logfile, $log, FILE_APPEND);
